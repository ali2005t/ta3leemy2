...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (emailForm) {
        emailForm.addEventListener('submit', async (e) => {
            ...
            if (!newEmail || !currentPass) return alert("يرجى ملء جميع الحقول");
            if (newEmail === currentUser.email) return alert("البريد الإلكتروني الجديد مطابق للحالي");
            ...
            try {
                ...
                if (error.code === 'auth/invalid-credential' || error.code === 'auth/wrong-password') {
                    ...
                    alert("يرجى تسجيل الخروج وإعادة الدخول للمتابعة");
                } else {
                    alert("حدث خطأ: " + error.message);
                }
            } finally {
