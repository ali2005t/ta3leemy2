...
document.addEventListener('DOMContentLoaded', () => {
    ...
    function renderRow(data, index) {
        ...
        if (data.type === 'book' || data.type === 'pdf') {
            typeBadge.innerHTML = '<i class="fas fa-file-pdf"></i> ملزمة';
            typeBadge.className = 'badge badge-red'; // Assuming CSS class or inline style
            typeBadge.style.background = 'rgba(239, 68, 68, 0.15)';
            typeBadge.style.color = '#f87171';
        } else if (data.type === 'exam') {
            typeBadge.innerHTML = '<i class="fas fa-clipboard-list"></i> امتحان';
            typeBadge.className = 'badge badge-warning';
        } else {
            // Default to video/lecture
            typeBadge.innerHTML = '<i class="fas fa-video"></i> فيديو';
