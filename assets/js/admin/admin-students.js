...
async function loadData() {
    try {
        // Fetch Admin Permissions/Scope first
        const adminDoc = await getDoc(doc(db, "admins", auth.currentUser.uid));
        const adminData = adminDoc.exists() ? adminDoc.data() : {};
        const allowedTeachers = adminData.assignedTeachers || []; // Array of Teacher IDs (if empty -> Super Admin/All)
        ...
        if (teacherSelect) {
            ...
            teacherSnap.forEach(t => {
                // Only show in filter if allowed
                if (allowedTeachers.length === 0 || allowedTeachers.includes(t.id)) {
                    const d = t.data();
                ...
                // Still map names for display purposes even if filtered out? 
                // Better to map all for name resolution, but filter list later.
                teacherMap[t.id] = t.data().name;
            });
        }
        ...
        snapVal.forEach(d => {
            ...
            // SECURITY CHECK: Does this student belong to an allowed teacher?
            let isAllowed = false;
            if (allowedTeachers.length === 0) {
                isAllowed = true;
            } else {
                // Check currentPlatform or enrolledTeachers
                if (s.currentPlatform && allowedTeachers.includes(s.currentPlatform)) isAllowed = true;
                if (s.enrolledTeachers) {
                    // if any enrolled teacher is in allowed list
                    if (s.enrolledTeachers.some(tid => allowedTeachers.includes(tid))) isAllowed = true;
                }
            }

            if (!isAllowed) return; // Skip this student
            ...
            if (s.currentPlatform && teacherMap[s.currentPlatform]) {
                ...
                // Try to find a teacher name that is meaningful (e.g. one of the assigned ones)
                const visibleTeacher = s.enrolledTeachers.find(tid => teacherMap[tid]); // Any mapped
                s.teacherName = teacherMap[visibleTeacher] || 'مشترك';
...
document.getElementById('export-btn')?.addEventListener('click', () => {
    ...
    const csvRows = [];
    // Headers
    csvRows.push(["Name", "Email", "Phone", "Teacher", "Date"].join(","));

    // Data
    allStudents.forEach(s => {
        const date = s.createdAt ? new Date(s.createdAt.seconds * 1000).toLocaleDateString('en-CA') : '';
        const row = [
            `"${s.name || s.fullName || ''}"`,
            `"${s.email || ''}"`,
            `"${s.phone || ''}"`,
