...
function loadBillingHistory(uid, subData) {
    const tbody = document.getElementById('billing-history-body');
    if (!tbody) return;
    tbody.innerHTML = '';

    if (!subData || !subData.isActive || subData.plan === 'trial') {
        tbody.innerHTML = '<tr><td colspan="5" style="text-align:center; padding:20px; color:#94a3b8;">لا توجد فواتير سابقة (حساب تجريبي أو جديد)</td></tr>';
        return;
    }

    const rows = [];

    // 1. Current Active Invoice (Assumed Paid)
    // We treat the current subscription period as the latest "Paid Invoice"
    ...
    if (!amount || amount === '---') { // Fallback prices
        if (subData.plan === 'basic') amount = 200;
        if (subData.plan === 'pro') amount = 500;
        if (subData.plan === 'elite') amount = 1200;
    }
    ...
    // 2. Next Invoice Assessment
    const today = new Date();
    const daysLeft = Math.ceil((endDate - today) / (1000 * 60 * 60 * 24));

    if (daysLeft <= 5) {
        // Show Upcoming/Overdue Invoice
        rows.push({
            name: `تجديد الاشتراك (${subData.planLabel || subData.plan})`,
        ...
        // Update Account Standing Badge
        const badge = document.getElementById('account-standing-badge');
        if (badge) {
            if (daysLeft < 0) {
                badge.innerText = 'غير منتظم (متأخر)';
                badge.style.color = '#ef4444';
            } else {
                badge.innerText = 'منتظم';
    ...
    rows.forEach(row => {
        ...
        if (row.status === 'paid') {
            statusHtml = '<span class="badge badge-success" style="background:#10b981; color:white; padding:4px 10px; border-radius:4px; font-size:0.8rem;">مدفوع</span>';
            actionHtml = `<button class="btn-sm" style="background:transparent; border:1px solid #64748b; color:#cbd5e1; border-radius:4px; cursor:not-allowed;" disabled><i class="fas fa-check"></i> تم الدفع</button>`;
        } else if (row.status === 'overdue') {
            statusHtml = '<span class="badge badge-danger" style="background:#ef4444; color:white; padding:4px 10px; border-radius:4px; font-size:0.8rem;">مستحق الدفع</span>';
            actionHtml = `<button onclick="initiateSubscription('${subData.plan}', '${subData.planLabel}', ${amount})" class="btn-sm" style="background:#6366f1; border:none; color:white; padding:5px 15px; border-radius:4px; cursor:pointer;"><i class="fas fa-credit-card"></i> ادفع الآن</button>`;
        } else if (row.status === 'upcoming') {
            statusHtml = '<span class="badge badge-warning" style="background:#f59e0b; color:black; padding:4px 10px; border-radius:4px; font-size:0.8rem;">قريباً</span>';
            actionHtml = `<button onclick="initiateSubscription('${subData.plan}', '${subData.planLabel}', ${amount})" class="btn-sm" style="background:transparent; border:1px solid #6366f1; color:#6366f1; padding:5px 15px; border-radius:4px; cursor:pointer;">تجديد مبكر</button>`;
        }

        tr.innerHTML = `
            <td style="padding:15px; color:white; font-weight:bold;">${row.name}</td>
            <td style="padding:15px; color:#cbd5e1;">${row.date}</td>
            <td style="padding:15px; color:white;">${row.amount}</td>
    ...
    // End of loadBillingHistory logic
}
...
async function calculateUsage(uid, currentTier) {
    ...
    if (pricingConfig && pricingConfig[currentTier]) {
        ...
        else if (currentTier === 'elite') { limitStudents = 0; limitStorage = 500; }
        else if (currentTier === 'trial' || currentTier === 'free_trial') { limitStudents = 50; limitStorage = 5; }
        else { limitStudents = 50; limitStorage = 5; } // Unknown -> Safe Default 5GB
    }
    ...
    try {
        ...
        // Fallback or use cached
        currentStudents = currentTeacherDoc.studentCount || 0;
    }

    // 3. Real Storage Calculation (No More Mocking!)
    const usedBytes = currentTeacherDoc.storageUsed || 0;
    let currentStorage = (usedBytes / (1024 * 1024 * 1024)).toFixed(2); // Provide 2 decimal places
