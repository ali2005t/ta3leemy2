...
export async function applyTheme(forcedTeacherId = null) {
    ...
    try {
        ...
        if (snap.exists()) {
            ...
            if (title) {
                const appTitles = document.querySelectorAll('.app-title, h1.title, .auth-header h2, #platform-name, #hero-platform-name, #header-platform-title, .logo');
                appTitles.forEach(t => t.innerText = t.classList.contains('auth-header') ? `مرحباً بك في ${title}` : title);
                document.title = title;
            }

            // 3. Logo Image
            // Targets: Landing large logo, Center card logo, Header logo
            const logoDivs = document.querySelectorAll('.training-logo-large, #center-card > div, #header-logo');

            if (logoDivs.length > 0 && (data.logo || data.profileImage || data.image)) {
                ...
                logoDivs.forEach(div => {
                    if (div.tagName === 'IMG') {
                        ...
                        // Keep clear background for header logo to avoid circle overlap issues if transparent
                        div.style.background = 'transparent';
                        div.innerHTML = `<img src="${src}" style="width:100%; height:100%; border-radius:50%; object-fit:cover;">`;
                    }
                });
            }

            // 4. Landing Page Layout
            if (data.appSettings && data.appSettings.landingLayout) {
                const layout = data.appSettings.landingLayout;
                // Remove existing layout classes if any
                document.body.classList.remove('layout-v1', 'layout-v2', 'layout-v3');
                // Add new one
                if (layout !== 'v1') {
